<div align="center"> <h1>🏔️ Snowpole Snow Depth Estimation Tool ❄️</h1> <p><i>Automated snow depth extraction from trail cameras using Computer Vision (RF-DETR)</i></p> </div> <hr>
👋 Welcome, Students!
This script processes a time series of trail camera images to automatically detect snow poles and estimate the depth of the snow. Instead of manually clicking through hundreds of images, this tool uses a fine-tuned RF-DETR (Robust Feature Detection Transformer) computer vision model to identify the visible portion of the snow pole, calculates the snow depth based on a physical conversion factor, and exports the data straight to a CSV!

✨ Features
🤖 AI-Powered: Uses a state-of-the-art transformer model to segment and measure snow poles.
🖱️ Interactive Setup: A terminal wizard walks you through setting up your camera metadata (no hard-coding required!).
